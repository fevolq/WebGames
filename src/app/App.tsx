...
import { games, gamePath, validateCatalog, type Game } from '../catalog/games';
...
export function StatusPage({ kind, game }: { kind: 'unavailable' | 'not-found' | 'error' | 'loading'; game?: Game }) {
  ...
  const description = kind === 'not-found' ? '地址可能有误。回到大厅，看看其他小小的快乐。'
    : kind === 'error' ? '请检查网络后刷新页面，或先返回大厅。'
    : kind === 'loading' ? '稍等一下，马上就好。'
    : maintenance ? '我们正在调整游戏体验，稍后再来看看吧。'
    : '这是一张游戏预告，具体玩法尚未开放。先到大厅逛逛吧。';
  const badge = kind === 'not-found' ? '404' : kind === 'error' ? 'OOPS' : kind === 'loading' ? '…' : maintenance ? 'PAUSE' : 'SOON';
  return <main id="main-content" tabIndex={-1} className={styles.statusPage}>
    ...
    {kind !== 'loading' && <div className={styles.statusActions}>
...
class GameErrorBoundary extends Component<{ children: ReactNode }, { failed: boolean }> {
  ...
  render() { return this.state.failed ? <StatusPage kind="error" /> : this.props.children; }
}
...
function GameRoute({ game }: { game: Game }) {
  ...
  return <GameErrorBoundary key={game.slug}>
