...
当前完成的是大厅：原创 SVG 封面、分类筛选、名称与标签搜索、URL 筛选状态、封面失败回退、游戏预告页与 404 页面。六款游戏均为标明「演示」「即将上线」的目录示例，尚未实现游戏玩法。
...
状态支持 `available`、`coming-soon` 和 `maintenance`；未开放与维护中的游戏禁用进入按钮，直接访问时展示对应提示。所有模块通过 `import.meta.glob` 延迟导入，游戏逻辑不会在打开大厅时执行。
